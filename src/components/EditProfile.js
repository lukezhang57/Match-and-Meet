...
const EditProfile = (props) => {
  ...
  useEffect(() => {
    ...
    const promise = load(
      `http://127.0.0.1:5000/Home/EditProfile${window.localStorage.getItem(
        "token"
      )}`
    );
    ...
    function myDisplayer(some) {
      console.log(some);
      setUsername(some[0][0]);
      setPassword(some[0][3]);
      setFirstName(some[0][1]);
      setLastName(some[0][2]);
      setMajor(some[0][4]);
      setResidence(some[0][5]);
      const interest_list = [];
      for (let i = 0; i < some[1].length; i++) {
        interest_list.push(some[1][i][2]);
      }
      setInterests(interest_list);

      const rso_list = [];
      for (let i = 0; i < some[2].length; i++) {
        rso_list.push(RSOList[some[2][i][0]]);
      }
      setRSOs(rso_list);
      var array = some[0][10].split(",").map(String);
      setCurrentClasses(array);
      // for(let i) {

      // }

      // document.getElementById("major-select").innerHTML = some[0][4];
      // document.getElementById("interest-select").innerHTML = some[0][6];
      // document.getElementById("major-select").innerHTML = some;
      // document.getElementById("major-select").classList.toggle("show");
      //frame.columns = ["NetID", "FirstName", "LastName", "Password", "major", Residence, major Id, current courses, interestid, rso id]
    }
  ...
  const handleMajorChange = (e) => {
    //document.getElementById("major-select").innerHTML = "";
    // document.getElementById("major-select").innerHTML = x;
    setMajor(e.target.value);

    //document.getElementById("major-select").value = e.target.value;
    //document.getElementById("major-select").innerHTML = e.target.value;
    //document.getElementById("major-select").classList.toggle("show");
  };
  ...
  const onSubmit = async () => {
    ...
    // Update database
    try {
      const response = await fetch("http://127.0.0.1:5000/Home/EditProfile", {
        method: "POST",
        headers: {
  ...
  // data.username = window.localStorage.getItem("token");
  ...
  return loggedIn ? (
    <div className="centered">
      <Stack
        ...
        p={6}
      >
        <div className="update-meet">
          <Typography variant="h3">Update User - Match and Meet</Typography>
        </div>
        <TextField
          id="first_select"
          label="First Name"
